...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for entity_id, entity_loader in tqdm(summ_dls.items()):
            ...
            assert len(texts)==distances.size(0)
            # Form ranked kl divergence list
            kl_divs = []
            dist_list=distances.detach().cpu().numpy()
            for i in range(distances.shape[0]):
                D_z = distances[i]
                kl_divs.append(kl_div_all_heads(D_z, P_z))
                #kl_divs.append(square_div_all_heads(D_z,P_z))
            ...
            all_indices[entity_id]=ranked_sentence_indices
    ...
    for entity_id, ranked_sentences in tqdm(ranked_entity_sentences.items()):
        ...
        summary_sentences,summ_ids = truncate_summary(
            ranked_sentences,
