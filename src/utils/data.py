...
class ReviewDataset(Dataset):
    ...
    def __init__(self,
        ...
        for entity_data in data:
            ...
            if 'split' in entity_data.keys():
                self.has_splits=True
            if 'split' in entity_data and (not training):
            ...
            for review_data in entity_data['reviews']:
                ...
                if 'rating' in review_data:
                    try:
                        label = int(float(review_data['rating'])) - 1
                    except:
                        pass
                else:
                    label = 0
                ...
                if training and self.has_splits:
                    if entity_data['split']=='train':
                        self.train_ids.append(full_id)
                    else:
                        self.dev_ids.append(full_id)
...
class ReviewSummarizationDataset(ReviewDataset):
    ...
    def __init__(self,
        ...
        super(ReviewSummarizationDataset,
              self).__init__(data, sample_size=sample_size, spmodel=spmodel, max_sen_len=max_sen_len,
                             max_rev_len=max_rev_len)
