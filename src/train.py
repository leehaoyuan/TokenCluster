        ...
        if (not args.no_kmeans) and epoch == args.warmup_epochs:
            with torch.no_grad():
                ...
                for i, batch in enumerate(train_dl):
                    ...
                    sent_mask=(src==pad_id)
                    sent_mask=torch.all(sent_mask,dim=2)
                    sent_mask=sent_mask.unsqueeze(2).repeat(1,1,args.output_nheads)
                    sent_mask=sent_mask.reshape(-1)
                    sent_mask=~sent_mask
                    out=out.reshape(-1, args.d_model)
                    out=out[sent_mask]
